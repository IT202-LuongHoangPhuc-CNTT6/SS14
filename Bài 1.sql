...
-- DATABASE LẤY TRÊN ĐỀ NHA THẦY , EM LIÊM 
...
-- Phần A: Phân tích
-- 1. Viết code tái hiện lỗi 
CALL PayHospitalFee(1, 100000);

-- 2. viết lệnh SELECT kiểm tra bảng Wallets và Patient_Invoices.
SELECT patient_id, balance 
FROM Wallets 
WHERE patient_id = 1;

SELECT patient_id, total_due 
FROM Patient_Invoices 
WHERE patient_id = 1;

-- Giải thích 
-- Sự cố "tiền đã trừ nhưng nợ không giảm" thì đang vi phạm nguyên tắc atomicity là tính nguyên tử của Transaction ( đại khái là nguyên tắc này chỉ cho phép tất cả cùng hoạt động được hoặc là không có gì hết) 
-- Nên khi có một bước thất bại thì toàn bộ quá trình phải bị hủy bỏ để trả dữ liệu về trạng thái ban đầu.

-- Phần B: Sửa chữa mã nguồn
-- DROP  thủ tục cũ nếu cos
DROP PROCEDURE IF EXISTS PayHospitalFee;

--  Viết lại thủ tục PayHospitalFee mới.
DELIMITER //
